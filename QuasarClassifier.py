...
def ImportImages(ImageFilenames, image_width, image_height):
    ...
    image_array = np.ndarray(
        shape=(len(ImageFilenames), 30, 30, 3), dtype=np.float32)
    image_num = 0
    ...
    for image in ImageFilenames:
        ...
        try:
            image_data = (ndimage.imread(image).astype(
                float) - 255.0 / 2) / 255.0
...
def QuasarClassifier(ImageFilenames, image_width, image_height):
    ...
    try:
        if image_height < 30:
            raise Exception(
                'Invalid image_height. Images must be at least 30 pixels by 30 pixels.')
        if image_width < 30:
            raise Exception(
                'Invalid image_height. Images must be at least 30 pixels by 30 pixels.')
        ...
        with tunedinceptiongraph.as_default():
            ...
            input_batch_normal = tf.placeholder(tf.float32, shape=(
                step_size, image_size, image_size, num_channels))
            input_batch_small = tf.placeholder(tf.float32, shape=(
                remaining_size, image_size, image_size, num_channels))
            ...
            with tf.name_scope('biases'):
                layer1_biases = tf.Variable(
                    tf.zeros([depth]), name='layer1_biases')
            ...
            def inceptionconvnet(data):
                ...
                with tf.name_scope('MaxPooling'):
                    ...
                    tf.summary.histogram(
                        'activation_MaxPooling', hiddenconv_4_1x1)
                ...
                with tf.name_scope('Conv1x1'):
                    ...
                    tf.summary.histogram(
                        'activation_Conv1x1', hiddenconv_3_1x1)
                ...
                with tf.name_scope('FullConn'):
                    ...
                    # Output layer
                    output = tf.matmul(
                        hidden3, outlayer_weights) + outlayer_biases
                    tf.summary.histogram('activation_output', output)

                return output

            # Output for batch data
            normal_prediction = tf.nn.softmax(
                inceptionconvnet(input_batch_normal))
            small_prediction = tf.nn.softmax(inceptionconvnet(
                input_batch_small))  # for the last batch of images
        ...
        for i in range((input_images.shape[0] // step_size) + 1):
            input_batch.append(
                input_images[i * step_size:(i + 1) * step_size, :, :, :])
        ...
        with tf.Session(graph=tunedinceptiongraph) as session:
            def predict():
                ...
                for i in range(len(input_batch) - 1):
                    preds.append(normal_prediction.eval(
                        feed_dict={input_batch_normal: input_batch[i]}))
                preds.append(small_prediction.eval(
                    feed_dict={input_batch_small: input_batch[len(input_batch) - 1]}))
